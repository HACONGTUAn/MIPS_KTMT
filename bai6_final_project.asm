.data
	...
	input_col:	.asciiz "\nNhap j (so thu tu cua dong): "
	input_val:	.asciiz "\nNhap gia tri gan cho phan tu mang : "
	output_val:	.asciiz "\nGia tri tra ve : "
	address_str:	.asciiz "\nDia chi cua bien con tro CharPtr | BytePtr | WordPtr | ArrayPtr l�: "
	value_str:	.asciiz "\nDia chi cua bien con tro CharPtr | BytePtr | WordPtr | ArrayPtr l�: "
	malloc_str:	.asciiz "\nBo nho da cap phatt: "
	bytes_str:	.asciiz " bytes"
	input_str:	.asciiz "\nNhap vao mot xau ki tu: "
	malloc_success:.asciiz "\nCap phat bo nho thanh cong. Mang bat dau tai dia chi: "
	mal_error:	.asciiz "\nError: So hang hoac so cot < 1000"
	bound_error:	.asciiz "\nError: Ngoai vung bo nho cho phep cua mang"
	null_error:	.asciiz "\nError: Chua khoi tao mang"
	overflow_error:.asciiz "\nError: Gia tri input qua lon (> 1000)"
	zero_error:	.asciiz "\nError: Gia tri input phai > 0"
	string_copy:	.space	100		# x�u copy
.kdata
...
malloc:
	...
	nop
	addi 	$t8, $t8, 3			
	andi 	$t8, $t8, 0xfffffffc 	# 0xfffffffc = 1111 1100 , khi and voi n� 
						# th� $t8 lu�n dua ve gi� tri gan nhat chia het cho 4 
skip:
...
#------------------------------------------
# kiem tra gia tri nhap vao >0 va <1000
#------------------------------------------
check_input:
	bge 	$a0, 1000, too_big
	nop
